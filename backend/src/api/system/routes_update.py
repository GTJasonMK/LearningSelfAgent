...
from backend.src.services.system.system_update import create_update_record
from backend.src.services.system.system_update import list_update_records as list_update_records_repo
from backend.src.services.system.system_update import update_update_record
...
def list_update_records(limit: int = DEFAULT_PAGE_LIMIT) -> dict:
    rows = list_update_records_repo(limit=clamp_page_limit(limit, default=DEFAULT_PAGE_LIMIT))
    return {
        "items": [
            {
                "id": row["id"],
