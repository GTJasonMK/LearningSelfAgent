...
AGENT_DOMAIN_PICK_PROMPT_TEMPLATE: Final = (
    ...
    "只输出 JSON（不要代码块、不要解释）：{{\"domain_ids\":[\"data\",\"web\"],\"reasons\":[{{\"domain_id\":\"data\",\"reason\":\"简短原因\",\"confidence\":0.8}}]}}\n"
    "规则：\n"
    "1) 优先选择与任务直接相关的领域（例如：数据处理任务选 data.*，网页抓取选 web.*）。\n"
    "2) 若任务跨多个领域，选择最相关的 1-3 个领域。\n"
    "3) 若没有合适领域或任务属于通用类型，输出 [\"misc\"]。\n"
    "4) 可附带 reasons（每个 domain_id 的选择理由与置信度），便于后续审计与调试。\n"
    "\n"
    "用户任务：\n"
    "{message}\n"
    "\n"
...
KNOWLEDGE_SUFFICIENCY_PROMPT_TEMPLATE: Final = (
    ...
    "只输出 JSON（不要代码块、不要解释）：\n"
    "{{\"sufficient\":true/false,\"reason\":\"简短原因\",\"missing_knowledge\":\"缺失的知识类型（如有）\",\"suggestion\":\"建议动作（如有）\",\"evidence\":[\"简短证据1\"]}}\n"
    "\n"
    "判断规则：\n"
    ...
    "5) evidence 至少给出 1 条可核对依据（例如：命中技能数量、命中图谱约束、关键记忆是否存在）。\n"
    "6) 只有在缺少用户私有/本地/授权信息时，才建议 ask_user。\n"
    "7) 对于可通过公开网页/API/搜索自行获取的信息任务（如搜索、抓取、收集、调研、整理公开数据），即使暂时缺少来源或口径细节，也优先给出 proceed，而不是 ask_user。\n"
    "8) 输出路径、候选来源、抓取顺序、默认频率等执行细节通常应由规划与执行阶段自行收敛，不应单独要求用户确认。\n"
    "\n"
    "用户任务：\n"
    "{message}\n"
    "\n"
...
AGENT_GRAPH_PICK_PROMPT_TEMPLATE: Final = (
    ...
    "只输出 JSON（不要代码块、不要解释）：{{\"node_ids\":[1,2],\"reasons\":[{{\"node_id\":1,\"reason\":\"简短原因\",\"confidence\":0.8}}]}}\n"
    "规则：\n"
    "1) 优先选择能直接影响当前任务的事实/约束/依赖（例如：路径、接口约定、已验证结论、用户偏好）。\n"
    "2) 若没有合适节点，输出空数组。\n"
    "3) 可附带 reasons（每个 node_id 的选择理由与置信度），便于后续审计与调试。\n"
    "\n"
    "用户任务：\n"
    "{message}\n"
    "\n"
...
AGENT_SKILL_PICK_PROMPT_TEMPLATE: Final = (
    ...
    "只输出 JSON（不要代码块、不要解释）：{{\"skill_ids\":[1,2],\"reasons\":[{{\"skill_id\":1,\"reason\":\"简短原因\",\"confidence\":0.8}}]}}\n"
    "规则：\n"
    "1) 仅选择能直接指导“怎么做”的技能（尤其是：如何用工具/命令获取外部信息、如何验证结果）。\n"
    "2) 若没有合适技能，输出空数组。\n"
    "3) 可附带 reasons（每个 skill_id 的选择理由与置信度），便于后续审计与调试。\n"
    "\n"
    "用户任务：\n"
    "{message}\n"
    "\n"
...
AGENT_SOLUTION_PICK_PROMPT_TEMPLATE: Final = (
    ...
    "只输出 JSON（不要代码块、不要解释）：{{\"solution_ids\":[1,2],\"reasons\":[{{\"solution_id\":1,\"reason\":\"简短原因\",\"confidence\":0.8}}]}}\n"
    "规则：\n"
    "1) 选择能直接复用的完整流程（包括：关键步骤顺序、工具使用、验证方式）。\n"
    "2) 若没有合适方案，输出空数组。\n"
    "3) 可附带 reasons（每个 solution_id 的选择理由与置信度），便于后续审计与调试。\n"
    "\n"
    "用户任务：\n"
    "{message}\n"
    "\n"
...
AGENT_MEMORY_PICK_PROMPT_TEMPLATE: Final = (
    ...
    "只输出 JSON（不要代码块、不要解释）：{{\"memory_ids\":[1,2],\"reasons\":[{{\"memory_id\":1,\"reason\":\"简短原因\",\"confidence\":0.8}}]}}\n"
    "规则：\n"
    "1) 优先选择能影响当前任务的长期信息：用户偏好/配置/Base URL/API Key 是否已配置/常用路径/已确认事实。\n"
    "2) 若没有合适记忆，输出空数组。\n"
    "3) 可附带 reasons（每个 memory_id 的选择理由与置信度），便于后续审计与调试。\n"
    "\n"
    "用户任务：\n"
    "{message}\n"
    "\n"
...
AGENT_PLAN_PROMPT_TEMPLATE: Final = (
    ...
    "2) title 要短、明确、可执行，并在标题里写清“动作/工具”（例如“tool_call:web_fetch 抓取 目标网址或关键词”或“file_write:test/sokoban/index.html 写入页面”）。\n"
    "2.1) title 只描述当前要执行的动作，不要把示例、假设来源、条件分支写进标题（例如不要写‘例如某站点/某代码’、‘若 web_fetch 不足则...’）。\n"
    ...
    "5.1.1) 但初始计划应优先写主路径，不要把‘如果现有工具失败，再去造工具’这种条件分支提前写成必经步骤。只有当已有工具已被真实观测证明不足时，才应在 replan/plan_patch 中补入工具自举链路。\n"
    "5.1.2) 对外部数据任务，若现有工具（如 web_fetch/http_request）尚未执行，初始计划优先安排‘现有工具抓取 -> 判断证据是否足够’；不要一开始就同时排入备用实现和自建工具主线。\n"
    ...
    "6.1) 对于“结构化数据产物（CSV/JSON/TSV 等）”任务，必须使用可执行链路处理数据：\n"
    "   - 推荐顺序：tool_call/http_request/file_read 获取原始数据 -> shell_command/tool_call 解析与换算 -> file_write 落盘 -> shell_command/tool_call 校验。\n"
    "   - llm_call 仅用于解释/总结，不得承担“行级数据提取、批量换算、批量生成 CSV/JSON 内容”。\n"
    "   - 写入结构化产物前，必须先拿到可验证的真实观测（例如：源文件片段、解析结果、脚本输出统计）。\n"
    "   - 若当前还没有真实样本，获取样本前最多只能准备一个抓取/读取脚本；不要先连续写多个解析/校验脚本。\n"
    "6.2) 若计划包含结构化数据产物，必须在 task_output 之前包含至少一步“验证/校验/检查/自测”步骤（allow 为 shell_command/tool_call/file_read 之一）。\n"
    "7) 计划必须包含“输出最终结果”的步骤（例如“输出结论”，allow=task_output）。\n"
    "8) artifacts 与 file_write.path 必须使用相对路径（相对 workdir）；禁止绝对路径（如 C:/... 或 /...）。\n"
    "9) 严禁编造数据：若要写入 CSV/JSON/报告等结果文件，必须先安排真实数据获取/解析步骤（tool_call/http_request/file_read/json_parse）并包含验证步骤。\n"
    "10) 当观测显示抓取/解析失败时，不得通过 file_write 写入模拟内容来“补齐” artifacts。\n"
    "11) json_parse 仅用于 JSON 字符串解析；网页 HTML/普通文本不要安排 json_parse，应改用 llm_call 或 shell_command/tool_call 解析。\n"
    "12) plan 中禁止出现普通 user_prompt；若确需用户补充，应由 knowledge sufficiency / pending_planning 阶段单独处理，而不是混入执行计划。\n"
    "\n"
    "用户指令：\n"
    "{message}"
)
...
AGENT_REACT_STEP_PROMPT_TEMPLATE: Final = (
    ...
    "payload 字段白名单（运行时权威，按 action.type）：\n"
    "{action_payload_keys_guide}\n"
    "说明：payload 只能使用上述字段；超出字段会被执行器丢弃。\n"
    "补充：task_output.content 默认必填；仅当“上一条且紧邻本步骤”的 llm_call 已生成可直接输出的结论时可留空。\n"
    "补充：tool_call.output 必须留空，由系统执行后填充。\n"
    "示例（仅示意）：\n"
    "- llm_call: {{\"prompt\":\"总结以下内容...\"}}\n"
    "- llm_call.prompt 只写分析指令，不要把已抓取的网页/CSV/JSON 原文再次粘贴进 prompt；系统会自动注入最近真实观测。\n"
    "- memory_write: {{\"content\":\"用户偏好中文输出\",\"tags\":[\"pref\"]}}\n"
    "- task_output: {{\"output_type\":\"text\",\"content\":\"完成说明...\"}}\n"
    "- tool_call: {{\"tool_name\":\"web_fetch\",\"input\":\"<target-url-or-query>\",\"output\":\"\"}}\n"
    "- shell_command: {{\"command\":\"python -c \\\"print(1)\\\"\",\"workdir\":\"{workdir}\"}}\n"
    "- script_run(等价于 shell_command alias): {{\"script\":\"backend/.agent/workspace/parse.py\",\"args\":[\"--input\",\"raw.json\",\"--output\",\"data/out.csv\"],\"required_args\":[\"--input\",\"--output\"],\"expected_outputs\":[\"data/out.csv\"],\"workdir\":\"{workdir}\"}}\n"
    "- file_write: {{\"path\":\"test/output.txt\",\"content\":\"hello\"}}\n"
    "- 脚本类 file_write 应先写最小可执行版本，避免一次生成超长 content 导致动作被截断。\n"
    ...
    "- http_request: {{\"url\":\"https://<api-endpoint>\",\"method\":\"GET\",\"timeout_ms\":10000,\"fallback_urls\":[\"https://<mirror-1>\",\"https://<mirror-2>\"]}}\n"
    "- json_parse: {{\"text\":\"{{\\\\\"a\\\\\":1}}\",\"pick_keys\":[\"a\"]}}\n"
    "- user_prompt: {{\"question\":\"请选择运行模式\",\"choices\":[{{\"label\":\"快速\",\"value\":\"fast\"}},{{\"label\":\"严谨\",\"value\":\"strict\"}}]}}\n"
    "\n"
    "重要规则：\n"
    "1) 如果需要实时/外部信息，优先使用工具获取（tool_call）而不是直接 llm_call。不要回答“无法访问”。\n"
    "   - 内置工具 web_fetch：input 可传 URL 或关键词，output 留空即可（系统会先尝试直连，必要时会执行“搜索 -> 候选页抓取”）。\n"
    "   - 示例：外部数据任务应先 tool_call(web_fetch/http_request) 抓取结构化数据，再 llm_call 总结，再 task_output 输出。\n"
    "     注意：示例来源仅作模式参考，不要把示例 URL 当作固定数据源。\n"
    "   - 若抓取结果为空、明显与目标无关，或缺少可验证的数据字段，你必须继续尝试其他来源（必要时用 plan_patch 把下一步改为 tool_call/shell_command 重试），禁止直接编造结论。\n"
    "   - 当使用 http_request 时，若主源可用性未知或近期失败，优先在同一步提供 fallback_urls（1-3 个候选），让执行器自动尝试可用源。\n"
    "   - http_request.url / fallback_urls 应优先来自用户输入、最近观测、已发现候选页或已验证 host；若当前还没有这些依据，先继续做来源发现，不要凭记忆猜 API。\n"
    "   - 若“最近外部源失败摘要”显示某来源出现 rate_limited/missing_api_key/tls_handshake_failed/http_403 等问题，"
    "下一次外部抓取必须切换来源族（至少切换 host，必要时同时切换协议或路径族），禁止原样重试同一 host。\n"
    "   - 外部抓取优先选择无需 API Key、返回结构化数据（JSON/CSV）的来源；若必须使用鉴权来源，先验证鉴权可用再继续。\n"
    "   - json_parse 仅用于解析 JSON 字符串；HTML/纯文本请使用 llm_call 总结或 shell_command/tool_call 做解析。\n"
    "   - json_parse.text 必须来自最近成功步骤的真实输出（tool_call/http_request/file_read/llm_call.response），且该输出应为 JSON；禁止手工粘贴长段静态文本冒充观测。\n"
    "   - 当目标是生成 artifacts 数据文件（如 csv/json）时，file_write.content 必须来源于真实观测或解析结果；若无真实数据，先继续抓取/解析，不得写入模拟数据。\n"
    "   - llm_call 不得直接产出“行级结构化数据文件内容”（例如整段 CSV/JSON 记录）；结构化数据转换应使用 shell_command/tool_call 执行脚本或工具完成。\n"
    ...
    "     tool_call.input 必须非空；创建工具/自测也要填入最小输入（例如 \"self_test\" / \"ping\" / 目标网址或关键词）。\n"
    ...
    "2.5) 如果缺少关键信息导致无法继续（例如需要用户确认/补充路径/选择项），你必须使用 user_prompt 询问用户，且问题要具体、可回答；若可枚举选项，请在 choices 中给出可点击候选。\n"
    "2.6) Windows 提示：shell_command 需要可执行文件；像 dir 这类 cmd 内建命令可能找不到。仅在“单行、无控制流”的简单检查下使用 python -c（例如 print/exists）。\n"
    "2.7) Windows 提示：python -c 中的路径请使用正斜杠（例如 C:/path/file.csv）或 raw string（r'...')，避免 \\t/\\n 等转义导致路径错误。\n"
    "2.8) 复杂 Python 逻辑（try/except/for/if/with/多语句）禁止直接用 python -c；必须先 file_write 脚本，再用 shell_command 执行脚本。\n"
    "2.8.1) 即便是简单校验，若包含 if/else/循环/异常处理，也应改为脚本文件执行，避免单行 python -c 语法脆弱。\n"
    "2.9) shell_command 执行本地脚本前，脚本必须在当前 run 中由 file_write/file_append 明确创建并落盘。\n"
    "2.9.0) 执行本地脚本时优先使用结构化 script_run 载荷（script+args+required_args+expected_outputs），避免自由字符串命令导致参数漂移。\n"
    "2.9.1) shell_command.command 仅支持单条命令（argv）语义：禁止使用 && / || / ; / | 链式连接；需要多步时请拆成多个计划步骤。\n"
    "2.10) 仅当本步骤是计划最后一步时才允许选择 task_output；否则禁止选择 task_output。\n"
    ...
    "注意：plan_patch.artifacts_add 仅允许相对路径，禁止绝对路径（如 C:/... 或 /...）。\n"
    "注意：若缺少真实数据证据，不要通过 file_write 直接产出看似完整的数据文件。\n"
    "\n"
    ...
    "  - artifacts_add：追加文件路径（仅相对路径；不要覆盖全量 artifacts）\n"
...
AGENT_REPLAN_PROMPT_TEMPLATE: Final = (
    ...
    "10) tool_call.input 必须非空；创建工具/自测也要填入最小输入（例如 \"self_test\" / \"ping\" / 目标网址或关键词）。\n"
    "11) 若 tool_metadata.exec 引用本地脚本路径，必须先 file_write 创建该脚本，再执行 tool_call。\n"
    "12) 若数据未获取成功，禁止把失败说明写入 artifacts；应继续重试或换实现。\n"
    "13) artifacts 只能使用相对路径（相对 workdir）；禁止绝对路径（如 C:/... 或 /...）。\n"
    "14) 严禁通过 file_write 直接编造模拟数据来“补齐” artifacts；应先重试抓取/解析并完成验证。\n"
    "15) 若“最近外部源失败摘要”非空，重规划必须体现来源多样化："
    "先换未失败 host，再做解析/总结；不得把同 host 的原样重试排在首个抓取步骤。\n"
    "16) 对外部数据任务，剩余计划应包含至少一个“备用来源”抓取路径（可用 insert_steps 或显式步骤标题体现）。\n"
    "17) 外部来源优先级：无需 API Key + 结构化响应 > 需要 API Key + 非结构化页面。\n"
    "17.1) 若当前失败来自源不可用/弱相关/候选为空，且最近观测里还没有可用具体 URL/host，下一步必须先做来源发现；不要直接编造新的 API URL 或站点路径。\n"
    "17.2) http_request 的 url/fallback_urls 应优先来自用户输入、最近观测、已发现候选页或已验证 host；若没有这些依据，不要把猜测 URL 放进首个抓取步骤。\n"
    "18) 重新规划阶段禁止插入普通 user_prompt；若任务仍无法自动推进，应继续换来源/换实现，或在最终失败时给出 unreachable proof，而不是中途 ask_user。\n"
    "19) 若“真实样本状态”显示 sample_available=no，则在首次获取样本前，最多只允许一个抓取/读取脚本 file_write；禁止先写解析脚本、校验脚本或结果文件。\n"
    "20) 若当前尚无真实样本，剩余计划必须尽快出现获取样本步骤（tool_call/http_request/file_read/shell_command）；不要先做 llm_call/json_parse/task_output。\n"
    "\n"
    "用户目标：{message}\n"
    "当前 workdir：{workdir}\n"
...
THINK_INITIAL_PLANNING_PROMPT_TEMPLATE: Final = (
    ...
    "8) artifacts 与 file_write.path 必须是相对路径（相对 workdir），禁止绝对路径。\n"
    "9) 若涉及数据文件 artifacts，必须包含“真实数据获取/解析 + 验证”步骤，禁止仅靠 llm_call/file_write 直接生成模拟数据。\n"
    "10) json_parse 步骤必须依赖前序真实观测，不要手工粘贴长段静态文本。\n"
    "\n"
...
THINK_IMPROVE_PROMPT_TEMPLATE: Final = (
    ...
    "\n"
    "规则：\n"
    "1) 输出必须是单个 JSON 对象，且首字符为 {{、尾字符为 }}。\n"
    "2) task_output 必须在最后一步。\n"
    "3) artifacts 与 file_write.path 必须使用相对路径（相对 workdir）。\n"
    "4) 若涉及数据文件 artifacts，必须包含“真实数据获取/解析 + 验证”步骤，禁止通过 llm_call/file_write 直接编造数据。\n"
    "5) json_parse 步骤必须依赖前序真实观测，不要手工粘贴长段静态文本。\n"
    "\n"
    "你的原方案：\n{original_plan}\n"
    "\n"
    "入选方案（参考）：\n{winning_plan}\n"
...
THINK_REFLECTION_FIX_PROMPT_TEMPLATE: Final = (
    ...
    "4) 若修复涉及数据文件/artifacts，必须先补“真实数据获取/解析”与“验证”步骤，禁止直接 file_write 模拟结果。\n"
    "5) insert_steps 中若包含 file_write，title 必须写成 file_write:<相对路径>，并使用相对路径。\n"
    "6) json_parse.text 必须来自前序真实观测，禁止粘贴静态大段文本。\n"
    "\n"
    "你的失败分析：\n{analysis}\n"
    "\n"
    "当前计划：\n{plan}\n"
...
AGENT_REACT_REVIEW_REPAIR_PROMPT_TEMPLATE: Final = (
    ...
    "   - 再继续抓取/处理/总结，为原计划末尾的 task_output 提供可验证输入\n"
    "6) 若修复涉及数据文件/artifacts，必须先补“真实抓取/解析 + 验证”步骤，禁止通过 file_write 直接编造结果。\n"
    "7) insert_steps 中新增 file_write 路径必须是相对路径；禁止绝对路径（如 C:/... 或 /...）。\n"
    "8) Windows 路径：python -c 中请使用正斜杠或 raw string（r'...')，避免 \\t/\\n 造成路径解析错误。\n"
    "\n"
    "用户目标：{message}\n"
    "当前 workdir：{workdir}\n"
    "实验目录（临时脚本/中间产物）：{agent_workspace}\n"
