...
AGENT_REACT_STEP_PROMPT_TEMPLATE: Final = (
    ...
    "- task_output: output_type(建议 text), content(默认必填；仅当“上一条且紧邻本步骤”的 llm_call 已生成可直接输出的结论时可留空，否则必须填写最终结论)\n"
    "- tool_call: tool_id/tool_name..., input(字符串且非空), output(必须留空；系统会执行工具并填充输出；禁止模拟/演示数据), tool_metadata.exec(新建/补全工具时必填), skill_id(可选)\n"
    ...
    "- http_request: url(必填), method, headers, params, data, json, timeout_ms, allow_redirects, encoding, max_bytes, strict_business_success(默认 true)\n"
    "- json_parse: text(必填, 且必须是合法 JSON 字符串), pick_keys(可选)\n"
    "- user_prompt: question(必填), kind(可选), choices(建议填写; 数组; 元素为 {{label,value}}；若问题存在有限选项，必须提供 2-5 个 choices 供前端点击)\n"
    "示例（仅示意）：\n"
