...
class TestReviewGateDecisionBranch(unittest.TestCase):
    def setUp(self):
    ...
    def test_review_gate_can_proceed_feedback_without_repair(self):
        ...
        with patch(
            ...
            return_value=(decision_text, None),
        ), patch(
        ...
        self.assertEqual(react_llm_mock.call_count, 1)
        result_titles = [s.title for s in result.plan_struct.steps]
        self.assertEqual(result_titles, ["输出结果", AGENT_TASK_FEEDBACK_STEP_TITLE])
