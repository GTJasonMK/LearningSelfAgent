...
class TestReviewGateInsertStepsTrim(unittest.TestCase):
    def setUp(self):
    ...
    def test_review_gate_insert_steps_without_max_steps_limit(self):
        ...
        llm_actions = [
            ...
            {"action": {"type": "task_output", "payload": {"output_type": "text", "content": "fixed1"}}},
            {"action": {"type": "task_output", "payload": {"output_type": "text", "content": "fixed2"}}},
        ]
        llm_side_effect = [
        ...
        self.assertEqual(len(result_titles), 4)
        self.assertEqual(result_titles, ["输出结果", "输出结果(修复-1)", "输出结果(修复-2)", AGENT_TASK_FEEDBACK_STEP_TITLE])
