...
class TestTaskPostprocessPassButDistillDenyKeepsDraftSkills(unittest.TestCase):
    def setUp(self):
    ...
    def test_postprocess_pass_but_distill_deny_should_not_promote_draft_skill(self):
        """
        验证：评估通过(pass)但 distill=deny 时，不应升级/沉淀本次 run 产生的 draft 技能。
        """
        ...
        with get_connection() as conn:
            cursor = conn.execute(
                ...
                ("pass 但不沉淀测试", "done", created_at, None, created_at, created_at),
            )
            ...
            cursor = conn.execute(
                ...
                (
                    ...
                    json.dumps({"mode": "do", "message": "pass 但不沉淀测试"}, ensure_ascii=False),
                ),
            )
        ...
        fake_review_json = json.dumps(
            {
                ...
                "pass_score": 90,
                "pass_threshold": 80,
                "distill": {"status": "deny", "score": 50, "threshold": 90, "reason": "一次性任务，不沉淀"},
                "summary": "ok",
                "issues": [],
                "next_actions": [],
                "skills": [],
        ...
        # 不应触发知识沉淀侧逻辑
        self.assertFalse(mock_solution_autogen.called)
        self.assertFalse(mock_skill_autogen.called)
