...
class TestAgentCommandStreamDoKnowledgeCreateDraftSkill(unittest.IsolatedAsyncioTestCase):
    """
    回归：do 模式知识充分性不足时触发 create_draft_skill 分支，仍能继续规划与执行收敛。

    覆盖点：
    - _assess_knowledge_sufficiency -> suggestion=create_draft_skill
    - _draft_skill_from_message 成功后创建 draft 技能并注入 skills_hint
    ...
    async def test_do_stream_create_draft_skill_branch_runs(self):
        from backend.src.agent.planning_phase import PlanPhaseResult
        from backend.src.agent.retrieval import DraftSkillResult, KnowledgeSufficiencyResult
        ...
        with patch(
            ...
            "backend.src.agent.runner.stream_new_run._draft_skill_from_message",
            ...
            return_value=("skills/misc/草拟技能.md", None),
        ), patch(
            # 有方案则不会触发 draft_solution
            ...
            async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
                async with client.stream(
                    ...
                    json={
                        "message": "test do create draft skill",
                        "mode": "do",
