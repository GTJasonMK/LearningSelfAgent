...
class TestReactErrorFailFast(unittest.TestCase):
    ...
    def test_default_non_retriable_codes(self):
        ...
        self.assertFalse(should_fail_fast_on_step_error("[code=missing_tool_exec_spec] 缺少执行定义"))
        self.assertFalse(should_fail_fast_on_step_error("[code=tool_exec_contract_error] 工具脚本未落盘"))
        self.assertFalse(should_fail_fast_on_step_error("[code=rate_limited] 上游限流"))
        # script_run / tool_call 契约错误应优先触发 replan，而非立即终止整轮执行。
